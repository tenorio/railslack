class ChatChannel < ApplicationCable::Channel
  ...
  def subscribed
    stream_from "#{params[:team_id]}_#{params[:type]}_#{@chat}" if authorize_and_set_chat
  end
  ...
  def authorize_and_set_chat
    if params[:type] == "channels"
      ...
      @record = Talk.find_by(user_one_id: [params[:id], current_user.id], user_two_id: [params[:id], current_user.id],
                             team: params[:team_id])
    end
